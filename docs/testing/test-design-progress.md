...
|-----|------|-------|-------------|------|
| TA-01 | R-01 | E2E (release smoke) | Install latest release, launch, check for update against a staged pre-release, verify signature, install, relaunch. Runs on every release tag. | GitHub Actions matrix on `win-latest` + `macos-latest` + `ubuntu-latest`; Tauri WebDriver + `tauri-plugin-updater` custom endpoint. |
